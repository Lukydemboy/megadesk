...
function collectGroups(): string[][] {
  return config.columns.flatMap((col) =>
    col.panes.map((p) =>
      p.agentIds.filter((id) => config.agents.some((a) => a.id === id)),
    ),
  );
}
...
export function applyPreset(shape: number[]) {
  ...
  let cursor = 0;
  mutate((c) => {
    c.columns = shape.map((paneCount): Column => ({
      frac: 1 / shape.length,
...
export function targetPaneRef(): PaneRef {
  const fp = focusedPane();
  if (fp && config.columns[fp.ci]?.panes[fp.pi]) return fp;
  for (let ci = 0; ci < config.columns.length; ci++) {
    const panes = config.columns[ci].panes;
    for (let pi = 0; pi < panes.length; pi++)
      if (panes[pi].agentIds.length === 0) return { ci, pi };
  }
  return { ci: 0, pi: 0 };
}
...
export function selectTabByIndex(n: number) {
  ...
  const i =
    n === 9 ? pane.agentIds.length - 1 : Math.min(n - 1, pane.agentIds.length - 1);
  const id = pane.agentIds[i];
  if (!id || id === pane.activeId) return;
  mutate((c) => {
    c.columns[fp.ci].panes[fp.pi].activeId = id;
  });
  saveConfig();
